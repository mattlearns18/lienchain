  ...
  return (
    <div className="ap-overlay" onClick={handleClose}>
      <div className="ap-modal rm-modal-wide" onClick={e => e.stopPropagation()}>
        ...
        {phase === "confirm" && (
          <>
            <h3 className="ap-modal-title">Request Lien Reduction</h3>
            ...
            {showPreview && (
              <div className="rm-preview-card">
                ...
                {[
                  [`New LienCo Share (${split}%)`,    usd(newLienCoAmt), "value"],
                  [`New Clinic Share (${clinicShare}%)`, usd(newClinicAmt), "value"],
                  ["Total Reduction", `${usd(reduction)} (${reductionPct}% reduction)`, "amber"],
                ].map(([label, value, type]) => (
                  <div key={label} className="ap-info-row">
                    <span className="ap-info-label">{label}</span>
                    <span className={`ap-info-value${type === "amber" ? " rm-amber" : ""}`}>{value}</span>
                  </div>
                ))}
              </div>
            )}
        ...
        {(phase === "running" || phase === "done") && (
          <>
            <h3 className="ap-modal-title">
              {phase === "done" ? "Request Submitted" : "Submitting…"}
            </h3>
            <p className="ap-modal-sub">{caseId}</p>
            ...
            {phase === "done" && (
              <>
                ...
                <p className="ap-modal-sub">You'll hear back within 1 business day via email.</p>
