...
**Phase 3, Phase 4, Phase 5, Phase 6, Phase 7, and Phase 8 are complete.**
...
**Next queued build: Phase 9 — Mainnet readiness.** The biggest engineering lift of the four. Touches: real on-chain settlement transactions (replacing the mock hashes the current sequential settlement flow produces; the actual payment-movement model — fiat off-chain attorney→LienCo + on-chain LienCo→clinic vs. fully on-chain three-leg — is an open architectural question for Matt to decide), mainnet seed handling separate from the testnet env var, feature flag for testnet-vs-mainnet so demos can stay on testnet while production runs on mainnet, partial-failure recovery for multi-clinic sequential settlement (retry per failed clinic), and a pre-flight checklist artifact (`MAINNET-READINESS.md`) covering the §7 business gaps (LLC, bank, E&O, opinion letter) before the first mainnet lien. July 1 target. Gated by business gaps; engineering work can proceed in parallel behind the feature flag.
