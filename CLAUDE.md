...
**Phase 3, Phase 4, Phase 5, Phase 6, and Phase 7 are complete.**
...
**Next queued build: Phase 8 — Attorney provisioning / invite flow.** Today `/attorney/:caseId` works but there's no way to actually provision an attorney to a case or send them a unique portal URL. Phase 8 adds: assign attorney to case at intake (or after), generate a unique attorney portal URL, simple email-link delivery (mailto: for now), basic attorney session (token in URL — no real auth yet, that arrives in Phase 9). Unblocks live demos with real PI attorneys in KC/STL. After Phase 8 the sequence is: Phase 9 mainnet-readiness (real on-chain settlement, mainnet seed handling, partial-failure recovery, feature flag for testnet-vs-mainnet, gated by business gaps in §7). July 1 target for Phase 9 ship.
