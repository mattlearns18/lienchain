...
- **`TransferFee` decision needed before mainnet volume:** `NFTokenMint` supports a `TransferFee` (0–50%) paid to the **issuer** on secondary sales — a possible on-chain enforcement path for the 1.5% `PLATFORM_FEE_PCT` that `money.js` currently books as record-keeping only. It requires `tfTransferable` and is **also immutable at mint**. Decide before minting at volume, even if implementation comes later.
